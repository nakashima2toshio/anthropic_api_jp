...
class UIHelper:
    ...
    def select_model(key: str = "model_selection", category: str = None, show_info: bool = True) -> str:
        ...
        selected = st.sidebar.selectbox(
            ...
            index=default_index,
            key=key,
            help="利用するAnthropic Claudeモデルを選択してください"
        )
    ...
    def show_settings_panel():
        ...
        with st.sidebar.expander("⚙️ 設定", expanded=False):
            ...
            # API キー設定
            st.write("**API キー設定**")
            current_key = config.get("api.anthropic_api_key") or os.getenv("ANTHROPIC_API_KEY")
            
            # APIキー表示（マスク済み）
            if current_key:
                masked_key = current_key[:10] + "*" * (len(current_key) - 20) + current_key[-10:] if len(current_key) > 20 else current_key[:5] + "*" * (len(current_key) - 5)
                st.text(f"現在のAPIキー: {masked_key}")
            else:
            ...
            new_api_key = st.text_input(
                ...
                help="Anthropic Console から取得したAPIキーを入力"
            )
            
            if st.button("APIキー更新", help="新しいAPIキーを設定"):
                if new_api_key and new_api_key.startswith("sk-ant-api"):
                    config.set("api.anthropic_api_key", new_api_key)
                    st.success("APIキーを更新しました")
                    st.rerun()
                elif new_api_key:
                    st.error("有効なAnthropic APIキー形式ではありません (sk-ant-api で始まる必要があります)")
                else:
                    st.warning("APIキーを入力してください")
            
            # APIキー接続テスト
            if st.button("接続テスト", help="現在のAPIキーで接続をテスト"):
                with st.spinner("接続テスト中..."):
                    try:
                        from helper_api import AnthropicClient
    ...
    def show_model_info(selected_model: str):
        ...
        with st.sidebar.expander("📊 モデル情報", expanded=False):
            ...
            if "sonnet" in selected_model:
                st.info("🎭 Claude Sonnet - バランス型高性能モデル")
            elif "haiku" in selected_model:
                st.info("⚡ Claude Haiku - 高速処理特化モデル")
            elif "opus" in selected_model:
                st.info("🧠 Claude Opus - 最高知能モデル")
            elif "claude-3-5" in selected_model:
                st.info("✨ Claude 3.5 - 最新世代モデル")
